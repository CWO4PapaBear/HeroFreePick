"""Activate the reviewed Starter login image. No SQL, DBC, MPQ or addon installation."""
...
def sql(query):return run(['docker','exec',DB,'sh','-c','MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot --batch --skip-column-names acore_characters -e "$1"','starter-login-read',query])
...
def main():
 ...
 expected={'modules/mod-hero-starting-path/src/StartingPath.cpp','src/server/game/Entities/Player/PlayerStorage.cpp'}
 ...
 def rollback():
  ...
  run(['docker','stop',WORLD]);copy_source('before');compose(work/'starter-login-rollback.json')
 ...
 if w['Image']==s['builtImage']:
  ...
  assert w['Config']['Labels']['com.docker.compose.project.config_files']==str(work/'starter-login-active.json'),'Installed image uses an unexpected compose configuration'
  ready(s['builtImage'],HERE/'already-active-check.log')
  print('ALREADY ACTIVE: Starter login image, source, ports and readiness verified. No restart or changes performed.')
  return
 assert w['Image']==s['previousImage'],'Live image changed since build; neither previous nor Starter login image is running. Stop for review.'
 ...
 if live_config!=s['activeConfig']:
  assert live_config==str(work/'starter-login-rollback.json'),'Unexpected recovery configuration'
 ...
 for name,data in [('rollback',old),('active',new),('private',private)]:
  path=work/('starter-login-'+name+'.json');save(path,data);run(['docker','compose','-p','classless-test','-f',str(path),'config','--quiet'])
 ...
 try:
  ...
  compose(work/'starter-login-private.json');ready(s['builtImage'],HERE/'private-startup.log')
  assert not inspect()['HostConfig']['PortBindings'],'Private startup unexpectedly published ports'
  compose(work/'starter-login-active.json');ready(s['builtImage'],HERE/'startup.log')
 ...
 print('ACTIVATION COMPLETE: Starter login saved starter ownership filter installed. No SQL migration or client installation. Obsolete saved starters will be reconciled on login. Previous image retained.')
