...
local function secondClass()
 ...
 for _,c in ipairs(A.classes)do if string.upper(c)~=own then local selected=c;i=i+1;local b=classChoices[c];if not b then b=A.CreateClassChoiceButton(box,c,function()finish(A.ChooseHybridPath(selected))end);classChoices[c]=b end;b:ClearAllPoints();b:SetPoint('TOPLEFT',30+((i-1)%5)*108,-122-math.floor((i-1)/5)*85);b:Show()end end
end
