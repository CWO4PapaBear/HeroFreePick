...
SecureUnitButton_OnLoad(f,'player',f.menu);f:RegisterForClicks('AnyUp')
f:SetSize(240,180);f:SetPoint('TOPLEFT',UIParent,'TOPLEFT',3,-2)
f:SetMovable(true);f:SetClampedToScreen(true)
local function playerDrag(surface)
 surface:RegisterForDrag('LeftButton')
 surface:SetScript('OnDragStart',function()
  local s=HeroPortraitSettings and HeroPortraitSettings.player
  if s and not s.locked and not InCombatLockdown()then f:StartMoving()end
 end)
 surface:SetScript('OnDragStop',function()
  if not InCombatLockdown()then
   f:StopMovingOrSizing();if HeroSavePortraitPosition then HeroSavePortraitPosition(f,'player')end
  end
 end)
end
playerDrag(f)
...
for _,d in ipairs(definitions)do
 ...
 playerDrag(b)
 b:SetScript('OnMouseUp',function(_,button)if button=='RightButton'and f.menu then f.menu()end end)
end
local combo={}
...
for i=1,2 do
 ...
 if i==1 then card.menu=function()ToggleDropDownMenu(1,nil,PetFrameDropDown,'cursor',0,0)end;SecureUnitButton_OnLoad(card,'pet',card.menu);card:RegisterForClicks('AnyUp');RegisterUnitWatch(card)end
...
local function updatePets(y)
 ...
 if UnitExists('pet')then
  ...
  local beasts={enUS='Beast',enGB='Beast',deDE='Wildtier',frFR='Bête',esES='Bestia',esMX='Bestia',ruRU='Животное',koKR='야수',zhCN='野兽',zhTW='野獸'}
  if UnitCreatureType('pet')==(beasts[GetLocale()]or 'Beast')and happiness and happiness>=1 and happiness<=3 then
...
local function update()
 ...
 local shared=A.HasSharedComboPoints and A.HasSharedComboPoints()
 local target=shared and UnitGUID('player')or UnitGUID('target');local points=demo and 5 or target and (GetComboPoints('player','target')or 0)or 0
 if shared then useCombo=true end
