"""Activate the reviewed Hero shared progression image. No SQL, DBC, MPQ or addon installation."""
...
def sql(query):return run(['docker','exec',DB,'sh','-c','MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot --batch --skip-column-names acore_characters -e "$1"','shared-progression-read',query])
...
def ready(image,path):
 ...
 for i in range(180):
  ...
  if (image!=build['builtImage'] or 'HERO_SHARED_PROGRESSION v1 ready;' in logs) and marker in logs and all(marker in logs for marker in ('HERO_STARTING_PATH ready v6.7;', 'HERO_DK_SCALING v1 ready;', 'MORE_MINIONS v0.2 staged handlers ready; shared Hunter stable', 'MORE_MINIONS independent Demon Mastery ready (native stable slot preserved)', 'HERO_PROJECTILES v0.4 loaded;')):
...
def main():
 ...
 expected={'src/server/game/Entities/Player/Player.cpp', 'modules/mod-hero-starting-path/src/SharedProgressionCurve.h', 'modules/mod-hero-starting-path/src/AdditionalResources.cpp', 'modules/mod-hero-starting-path/src/StartingPath.cpp', 'modules/mod-hero-starting-path/src/SharedProgression.cpp', 'modules/mod-hero-starting-path/src/SharedProgression.h'}
 ...
 def rollback():
  ...
  run(['docker','stop',WORLD]);copy_source('before');compose(work/'shared-progression-rollback.json')
  ready(s['previousImage'],HERE/'rollback.log');assert inspect()['HostConfig']['PortBindings']==s['ports']
  save(HERE/'activation.json',{'phase':'rolled-back','image':s['previousImage']});print('ROLLBACK COMPLETE. Previous image/source restored; class-based progression restored on login; earned levels, builds and gear retained.')
 if a.rollback:
 ...
 if w['Image']==s['builtImage']:
  ...
  assert w['Config']['Labels']['com.docker.compose.project.config_files']==str(work/'shared-progression-active.json'),'Installed image uses an unexpected compose configuration'
  ready(s['builtImage'],HERE/'already-active-check.log')
  print('ALREADY ACTIVE: Hero shared progression image, source, ports and readiness verified. No restart or changes performed.')
  return
 assert w['Image']==s['previousImage'],'Live image changed since build; neither previous nor Hero shared progression image is running. Stop for review.'
 ...
 if live_config!=s['activeConfig']:
  assert live_config==str(work/'shared-progression-rollback.json'),'Unexpected recovery configuration'
 ...
 for name,data in [('rollback',old),('active',new),('private',private)]:
  path=work/('shared-progression-'+name+'.json');save(path,data);run(['docker','compose','-p','classless-test','-f',str(path),'config','--quiet'])
 ...
 try:
  ...
  compose(work/'shared-progression-private.json');ready(s['builtImage'],HERE/'private-startup.log')
  assert not inspect()['HostConfig']['PortBindings'],'Private startup unexpectedly published ports'
  compose(work/'shared-progression-active.json');ready(s['builtImage'],HERE/'startup.log')
 ...
 print('ACTIVATION COMPLETE: Hero shared progression base attributes and health/mana curve installed. No SQL migration or client installation. Existing Hero/Hybrid characters recalculate on login; Class+ unchanged. EXPERIMENTAL BALANCE CHANGE. Previous image retained.')
