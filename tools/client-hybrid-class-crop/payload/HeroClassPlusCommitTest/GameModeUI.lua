...
function A.ShowModeUpgrade(mode,class)
 ...
 if target=='Hybrid'and not second then
  ...
  for _,name in ipairs(names)do if name:upper()~=own then local selected=name;i=i+1;local b=A.CreateClassChoiceButton(box,name,function()second=selected;review()end);b:SetPoint('TOPLEFT',20+((i-1)%5)*105+(i>5 and 52.5 or 0),-166-math.floor((i-1)/5)*86);rows[#rows+1]=b end end
