...
function A.CreateClassChoiceButton(parent,class,onClick)
 ...
 if SetPortraitToTexture then SetPortraitToTexture(portrait,icon)else portrait:SetTexture(icon)end
 ...
 b:SetScript('OnEnter',function(self)
  ...
  for i=0,10 do local line=classDescriptions[class:upper()..i];if line then GameTooltip:AddLine(line,1,1,1,true)end end
  GameTooltip:Show()
 end)
...
local function clear()for _,b in ipairs(classChoices)do b:Hide()end;classChoices={};upgradeNote:Hide();for _,b in ipairs(buttons)do b:Hide()end end
...
local function secondClass()
 ...
 for _,c in ipairs(A.classes)do if string.upper(c)~=own then local selected=c;i=i+1;local button=A.CreateClassChoiceButton(box,c,function()finish(A.ChooseHybridPath(selected))end);button:SetPoint('TOPLEFT',30+((i-1)%5)*108,-122-math.floor((i-1)/5)*85);classChoices[#classChoices+1]=button end end
end
