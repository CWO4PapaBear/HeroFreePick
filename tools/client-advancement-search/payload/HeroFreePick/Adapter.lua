...
function A.FilteredLearnedEntries()
 ...
 for _,e in ipairs(A.LearnedEntries())do
  ...
  local spec=(A.learnedQuery or ''):find('%S') or not A.spec or A.spec=='All' or (e.spec==A.spec and e.class==A.class)
  ...
  if rarity and plan and spec and kind and (A.SearchNameMatches and A.SearchNameMatches(e,A.learnedQuery)or not A.SearchNameMatches and string.find(string.lower(e.name),string.lower(A.learnedQuery or ''),1,true))then out[#out+1]=e end
 end
 return out
end
...
-- Shared custom-mode requirement: UI guards and server bridge call this helper.
function A.RequiresPrimaryStat()
 return A.mode=='ClassPlus'or A.mode=='Hybrid'or A.mode=='Hero'
end
function A.HasPrimaryStatSelection()
 local stat=HeroFreePickPlans and HeroFreePickPlans.primaryStat
 return stat=='Strength'or stat=='Agility'or stat=='Intellect'or stat=='Spirit'
end
function A.ValidatePrimaryStatSelection()
 if A.RequiresPrimaryStat()and not A.HasPrimaryStatSelection()then
  return false,'Choose a Primary Stat before accepting abilities or talents or closing Hero Advancement.'
 end
 return true
end
local acceptWithPrimary=A.AcceptPreparation
function A.AcceptPreparation()
 local ok,why=A.ValidatePrimaryStatSelection();if not ok then return false,why end
 return acceptWithPrimary()
end
