"""Activate the reviewed Hybrid equipment image. No SQL, DBC, MPQ or addon installation."""
...
def sql(query):return run(['docker','exec',DB,'sh','-c','MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot --batch --skip-column-names acore_characters -e "$1"','hybrid-equipment-read',query])
...
def main():
 ...
 expected={'modules/mod-hero-starting-path/src/'+n for n in ('AdditionalResources.cpp','AdditionalResources.h','StartingPath.cpp','EquipmentPolicy.h')}|{'src/server/game/Entities/Player/'+n for n in ('Player.cpp','PlayerStorage.cpp')}
 ...
 def rollback():
  ...
  run(['docker','stop',WORLD]);copy_source('before');compose(work/'hybrid-equipment-rollback.json')
 ...
 if w['Image']==s['builtImage']:
  ...
  assert w['Config']['Labels']['com.docker.compose.project.config_files']==str(work/'hybrid-equipment-active.json'),'Installed image uses an unexpected compose configuration'
  ready(s['builtImage'],HERE/'already-active-check.log')
  print('ALREADY ACTIVE: Hybrid equipment image, source, ports and readiness verified. No restart or changes performed.')
  return
 assert w['Image']==s['previousImage'],'Live image changed since build; neither previous nor Hybrid equipment image is running. Stop for review.'
 ...
 if live_config!=s['activeConfig']:
  assert live_config==str(work/'hybrid-equipment-rollback.json'),'Unexpected recovery configuration'
 ...
 for name,data in [('rollback',old),('active',new),('private',private)]:
  path=work/('hybrid-equipment-'+name+'.json');save(path,data);run(['docker','compose','-p','classless-test','-f',str(path),'config','--quiet'])
 ...
 try:
  ...
  compose(work/'hybrid-equipment-private.json');ready(s['builtImage'],HERE/'private-startup.log')
  assert not inspect()['HostConfig']['PortBindings'],'Private startup unexpectedly published ports'
  compose(work/'hybrid-equipment-active.json');ready(s['builtImage'],HERE/'startup.log')
 ...
 print('ACTIVATION COMPLETE: Hybrid equipment proficiencies and class eligibility installed. No SQL, DBC, MPQ or addon files changed. Previous image retained.')
