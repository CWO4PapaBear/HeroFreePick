"""Install only the reviewed portrait repair. Close WoW first."""
...
def main():
    ...
    addon=a.client/'Interface/AddOns/HeroClassPlusCommitTest'
    ...
    if not pending:
        print('PORTRAIT REPAIR ALREADY INSTALLED. No changes.');return
    backup=HERE/'backups'/datetime.datetime.now().strftime('%Y%m%d-%H%M%S-%f')
    backup.mkdir(parents=True)
    for name in pending:
        if (addon/name).exists():shutil.copy2(addon/name,backup/name)
    changed=[]
    try:
        for name in pending:
            tmp=addon/(name+'.portrait-new')
    ...
    print('PORTRAIT REPAIR COMPLETE. Player/target controls, pet menus, narrower bars and stock rune hiding installed.')
    print('Backup:',backup)
