...
local function allowed(text)
 if not second or not ITEM_CLASSES_ALLOWED then return false end
 ...
 for name in names:gmatch('[^,]+')do
  name=name:match('^%s*(.-)%s*$')
  if name==male[second]or name==female[second]then return true end
 end
 return false
end
local function update(tip)
 if not second or not tip.GetItem then return end
 local _,link=tip:GetItem()
 if not link then return end
 local equip=select(9,GetItemInfo(link))
 if not equip or equip=='' then return end
...
frame:SetScript('OnEvent',function(_,event,message)
 if event=='PLAYER_LOGIN'then second=nil;return end
 if message=='HF_PATH PENDING'then second=nil;return end
 if type(message)~='string'then return end
 local mode,id=message:match('^HF_MODE STATE ([1-4]) [01] (%d+)$')
 if not mode then return end
 mode,id=tonumber(mode),tonumber(id)
